# ObjectDetection guide installation
...
This step is just if you are using [Anaconda](https://www.anaconda.com/) distribution for your Data Science project.
...
If your are using [virtualenv](https://virtualenv.pypa.io/en/latest/) or [venv](https://docs.python.org/3/library/venv.html) this template is customized for them and in fact you have already created a virtual environment ``venv/``, you just need to access and install the packages with [pip](https://pypi.org/project/pip/)
...
To move beyond notebook prototyping, all reusable code should go into the `test/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `test` folder and use the modules inside your notebooks :
...
```python
from test.utils.paths import data_dir
data_dir()
```
